...
WHERE playlist_id IN (
  SELECT playlist_id FROM playlist WHERE name LIKE 'Music'
  );
...
FROM track
WHERE genre_id IN (
  SELECT genre_id FROM genre WHERE name LIKE 'Comedy'
  );

  SELECT *
FROM track
WHERE album_id IN (
  SELECT album_id FROM album WHERE name LIKE 'Fireball'
  );
